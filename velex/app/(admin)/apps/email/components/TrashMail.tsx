import React from "react";
import { BsTrash } from "react-icons/bs"; 
import { BiSolidTagAlt, BiTagAlt } from "react-icons/bi";

interface Email {
  id: number;
  sender: string;
  subject: string;
  content: string;
  important: boolean;
  date: string;
}

const TrashMail = () => {
  const emails: Email[] = [
    {
      id: 9,
      sender: "Dolly K. Sullivan",
      subject: "Let's go fishing!",
      content:
        "Hey, You wanna join me and Fred at the lake tomorrow? It'll be awesome.",
      important: false,
      date: "Feb 23",
    },
    {
      id: 10,
      sender: "Miriam R. Egan (7)",
      subject: "Hey man",
      content: "Nah man sorry i don't. Should i get it?",
      important: true,
      date: "Feb 23",
    },
    {
      id: 11,
      sender: "Lisa G. Nixon",
      subject: "1 new items in your Stackexchange inbox",
      content:
        "The following items were added to your Stack Exchange global inbox since you last checked it.",
      important: true,
      date: "Feb 21",
    },
    {
      id: 12,
      sender: "Thomas T. Russell",
      subject: "You can now use your storage in Google Drive",
      content:
        "Hey Nicklas Sandell! Thank you for purchasing extra storage space in Google Drive.",
      important: true,
      date: "Feb 20",
    },
    {
      id: 13,
      sender: "Erlinda A. Blair, me (3)",
      subject: "Hello",
      content:
        "Trip home from Colombo has been arranged, then Jenna will come get me from Stockholm. :)",
      important: false,
      date: "Mar 6",
    },
  ];

  return (
    <div
      //   className="tab-pane fade"
      id="email-trash"
      role="tabpanel"
      aria-labelledby="email-trash-tab"
    >
      <hr />
      <div className="text-center mt-2">
        <p className="mb-0">
          Messages that have been in Trash more than 30 days will be
          automatically deleted.
          <a href="javascript: void(0);" className="text-primary ms-2">
            Empty Trash Now
          </a>
        </p>
      </div>
      <hr className="mb-3" />

      <ul className="message-list mb-0">
        {emails.map((email) => (
          <li key={email.id}>
            <div className="col-mail col-mail-1">
              <div className="checkbox-wrapper-mail">
                <input type="checkbox" id={`TrashChk${email.id}`} />
                <label
                  htmlFor={`TrashChk${email.id}`}
                  className="toggle"
                ></label>
              </div>
              <span className="important-toggle">
                {email.important ? (
                  <BiSolidTagAlt className="text-warning" />
                ) : (
                  <BiTagAlt />
                )}
              </span>
              <span className="trash-toggle">
                <BsTrash />
              </span>
              <a
                data-bs-toggle="offcanvas"
                href="#email-read"
                className="title"
              >
                {email.sender}
              </a>
            </div>
            <div className="col-mail col-mail-2">
              <a
                data-bs-toggle="offcanvas"
                href="#email-read"
                className="subject"
              >
                {email.subject} &nbsp;&ndash;&nbsp;
                <span>{email.content}</span>
              </a>
              <div className="date">{email.date}</div>
            </div>
          </li>
        ))}
      </ul>
    </div>
  );
};

export default TrashMail;
